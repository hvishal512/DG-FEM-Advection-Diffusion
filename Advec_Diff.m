...
for i=1:length(saved)
    ...
    title('DG Advection-Diffusion Solution','Fontname','Arial','Fontsize',20);
    xlabel('x','Fontname','Arial','Fontsize',16);
    ylabel('u(x,t)','Fontname','Arial','Fontsize',16);
    pause(0.01);
    %F(i) = getframe(gcf);
end
%video = VideoWriter('Advec_diff1D.avi', 'Uncompressed AVI');
%open(video);
%writeVideo(video,F);
%close(video);